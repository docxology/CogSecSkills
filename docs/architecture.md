...
runs identically under Claude Code, Codex, or Hermes. This document describes
the system design: the three coherent artifacts, the runner package, the
harness-neutral contract, and the deterministic authoring pipeline.

> The deliverable is the skills system. The package under
> `src/cogsecskills/` is the small, fully-tested engine that discovers,
> parses, validates, authors, and reports on those skills. All logic lives in
> the package; the CLI and scripts only orchestrate (the thin-orchestrator
> contract).
...
| **BUILD** | *how* each skill works | multiharness skill folders | [`skills/<group>/<slug>/`](../skills/) |
...
- **PLAN — the registry.** `registry/skills.yaml` enumerates every skill area
  ...
  no on-disk folder.
- **BUILD — the skills tree.** Each implemented area is a folder under
  `skills/<group>/<slug>/` containing the harness-neutral spec plus its
  companion files (see [Anatomy](#anatomy-of-a-skill-build)). This is the
  *build*: the thing that actually ships and runs.
...
| [`harness.py`](../src/cogsecskills/harness.py) | Multiharness conformance. Defines `HARNESSES` (`claude`, `codex`, `hermes`) and `HARNESS_VERB_SUPPORT`. `check_conformance` proves a spec declares an adapter for each harness and that each harness can realise every verb the skill uses. The check is structural, not behavioural. |
...
| [`author.py`](../src/cogsecskills/author.py) | The deterministic renderer. `render_definition` turns a structured JSON definition into the six conforming files; adapters are generated to bind **exactly** the declared verbs. `author_batch` renders every `_def.json` and promotes the registry. `promote_to_implemented` flips registry status by targeted text edit. |
...
| [`cli.py`](../src/cogsecskills/cli.py) | Thin orchestrator. Parses args, calls the library API, prints results — `list`, `show`, `validate`, `report`, `route`, `stats`, `groups`, `catalogue`, `doctor`, `export`, `scaffold`, `author`, `author-batch`. No business logic. |
...
one adapter per harness under `harness/` — a Markdown binding table whose first
column lists the neutral verb and whose remaining columns name the concrete
tool. For example `read` binds to Claude Code `Read`/`Grep`, to Codex
`shell` (`cat`, `rg`), and to Hermes `fs.read`.
...
six conforming files). Workflow agents supply substance as a structured JSON
*definition*; `author.py` supplies format deterministically.

```
                 SUBSTANCE                          FORMAT
   ┌─────────────────────────────┐     ┌──────────────────────────────────┐
   │  structured JSON definition │     │   author.render_definition()      │
...
format stragglers no matter how many skills are authored in parallel. The 8
reference exemplars were hand-authored; the other ~92 areas were authored in
parallel (one agent per technique) as definitions and rendered this way.
...
- **90% coverage gate** on `src/`; the runner package sits near 96%.
